...
| upstreamToken | Optional **environment-only** bearer token injected into the upstream `Authorization` header only after a configured webhook HMAC validates. Configure it as `GWP_UPSTREAMTOKEN`; it requires an explicit HTTPS upstream with normal certificate and hostname verification. Every redirect is revalidated, and an HTTPS-to-HTTP same-host downgrade is rejected before it receives authorization. The HTTP exceptions are literal `http://127.0.0.1` or `http://[::1]` inside one process namespace, and a `*.railway.internal` name, which resolves only inside one Railway project's WireGuard-encrypted private network; neither accepts userinfo/query/fragment. Do not put it in a URL, command line, log, or repository. It must not equal `secret`. | | supplied by the runtime secret manager |
